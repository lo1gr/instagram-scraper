...
class InstagramScraper(object):
    ...
    def __query(self, url, variables, entity_name, query, end_cursor):
        # resp is not the right one for a single image: date is wrong, no accessibility caption, no tagged_users
    ...
    def augment_node(self, node):
        ...
        if 'urls' not in node:
            node['urls'] = []
            node['tagged_user'] = self.extract_tagged_users(node)
        if node['is_video'] and 'video_url' in node:
            ...
            if details:
                ...
                if '__typename' in details and details['__typename'] == 'GraphVideo':
                    ...
                    # new addition: get all the tagged users in carousel
                    node['urls'] = urls
                    node['tagged_user'] = self.extract_tagged_users(details)
                else:
    ...
    def __get_media_details(self, shortcode):
        """Goes into here only when GraphSidecar aka when the post is a collection of media."""
    ...
    def __query_media(self, id, end_cursor=''):
        # resp does not contain tagged user
        ...
        if resp is not None:
            ...
            if payload:
                # payload does not contain tagged_user
    ...
    def extract_tagged_users(self, item):
        """Extracts the tagged users from a post."""
        
        # for single images the item does not even appear here


        tagged_user_list = []
        item['tagged_user'] = '0'

        if 'edge_media_to_tagged_user' in item and item['edge_media_to_tagged_user'] and item['edge_media_to_tagged_user'][
            'edges'] and item['edge_media_to_tagged_user']['edges'][0]["node"]:
            for tagged_user in item['edge_media_to_tagged_user']['edges']:
                tagged_user_list.append(tagged_user['node']['user']['username'])
            item['tagged_user'] = list(set(tagged_user_list))


        return item['tagged_user']
